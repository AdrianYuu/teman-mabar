...
@section('title', 'Admin Dashboard | Create Game')

@section('content')

    <form action={{ route('storeGame') }} method="POST" class="max-w-8xl mx-auto" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
            ...
            <input type="text" id="name" name="name"
                ...
                placeholder="Input new game name..." value="{{ old('name') }}" />
        </div>
        ...
        <div class="mb-3">
            <label for="pricing_type_id" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Pricing Type</label>
            <select id="pricing_type_id" name="pricing_type"
                class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                <option value="" selected>Please select one...</option>
