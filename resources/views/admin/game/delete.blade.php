...
@section('title', 'Admin Dashboard | Delete Game')

@section('content')

    <form action={{ route('destroyGame', ['id' => $game->id]) }} method="POST" class="max-w-8xl mx-auto mb-4">
        @method('DELETE')
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="game_picture">Game
                Picture</label>
            <img src="{{ $game->game_picture_url }}" alt="game_picture" class="w-48">
        </div>
