...
@section('title', 'Player Detail Page')
...
@section('content')
    <div class="flex w-full flex-col gap-2 px-24">
        ...
        <div class="flex w-full gap-4">
            <div
                class="h-[36rem] max-w-sm rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800">
                <a href="#">
                    <img class="h-96 rounded-t-lg object-cover"
                        src="{{ $user->profile_picture_url ?? asset('assets/images/Profile-icon.jpg') }}" alt="" />
                </a>
                ...
                <div class="p-5">
                    ...
                    <p class="mb-3">{{ $user->biography ?? 'There is no biography.' }}</p>
                    <p>{{ $user->gender ?? 'Gender is unknown.' }}</p>
                </div>
            </div>

            <div class="flex w-full flex-wrap gap-6">
                @foreach ($user->userPriceDetails as $userPriceDetail)
                    <div
                        ...
                        <a href="#">
                            <img class="h-48 w-full rounded-t-lg object-cover"
                                src="{{ $userPriceDetail->game->game_picture_url ?? asset('assets/images/Profile-banner.jpg') }}"
                                alt="" />
                        </a>
                        <div class="p-5">
                            <a href="#">
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                                    {{ $userPriceDetail->game->name }}</h5>
                            </a>
                            ...
                            <div id="orderModal-{{ $userPriceDetail->user_id }}" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="relative mx-auto mt-24 w-full max-w-md">
                                    <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                                        <form
                                            action="{{ route('storeOrder', ['gameId' => $userPriceDetail->game_id, 'gamerUserId' => $userPriceDetail->user_id]) }}"
                                            method="POST" class="p-6">
                                            @csrf
                                            @if ($userPriceDetail->game->price_type === 'Match')
                                                <div class="mb-4">
