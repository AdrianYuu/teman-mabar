    ...
    <div class="flex w-full flex-col gap-2 px-24">
        ...
        <div class="flex w-full gap-4">
            <div
                ...
                <a href="#">
                    <img class="h-96 rounded-t-lg object-cover" src="{{ $game->game_picture_url }}" alt="" />
                </a>
                ...
                <div class="p-5">
                    ...
                    @if ($game->price_type === 'Hour')
                        <p>Kamu bisa bermain {{ $game->name }} dengan membayar per jam.</p>
                    @else
                        <p>Kamu bisa bermain {{ $game->name }} dengan membayar per tiap kali permainan.</p>
                    @endif
                </div>
            </div>

            <div class="flex w-full flex-wrap gap-6">
                @if ($game->userPriceDetails->filter(fn($userPriceDetail) => $userPriceDetail->user->id !== auth()->id())->isEmpty())
                    <div class="flex w-full items-center justify-center text-lg font-bold">Belum ada pemain yang memainkan
                        game ini.</div>
                @else
                    @foreach ($game->userPriceDetails->filter(fn($userPriceDetail) => $userPriceDetail->user->id !== auth()->id()) as $userPriceDetail)
                        <div
                            class="max-w-72 max-h-[22rem] rounded-lg border border-gray-200 shadow dark:border-gray-700 dark:bg-gray-800">
                            <a href="#">
                                <img class="h-48 w-48 rounded-t-lg object-cover"
                                    src="{{ $userPriceDetail->user->profile_picture_url ? $userPriceDetail->user->profile_picture_url : asset('assets/images/Profile-banner.jpg') }}"
                                    alt="" />
                            </a>
                            <div class="p-5">
                                <a href="#">
                                ...
                                <div class="mb-1 flex h-full">
                                    <svg class="h-10 w-10" viewBox="0 0 16 16" fill="none"
                                    ...
                                    <p class="mt-1.5 font-normal text-gray-700 dark:text-gray-400">
                                ...
                                <div class="mt-2 flex">
                                    <button data-modal-target="orderModal-{{ $userPriceDetail->user_id }}"
                                        ...
                                        class="flex w-full items-center justify-center rounded-lg bg-blue-700 px-3 py-2 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        Order
                                    </button>
                                </div>
                                <div id="orderModal-{{ $userPriceDetail->user_id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div class="relative mx-auto mt-24 w-full max-w-md">
                                        <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                                            <form
                                                action="{{ route('storeOrder', ['gameId' => $game->id, 'gamerUserId' => $userPriceDetail->user_id]) }}"
                                                ...
                                                @if ($game->price_type === 'Match')
                                                    <div class="mb-4">
                                                        <label for="total_match"
                                                            ...
                                                            Total Match</label>
                                                        <input type="number" id="total_match" name="total_match" required
